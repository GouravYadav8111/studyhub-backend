...
const authorizeRoles = require('../middleware/roleMiddleware'); // 👈 NEW: Imported the bouncer
const Library = require('../models/Library');

const crypto = require('crypto');
const Enrollment = require('../models/Enrollment');

// --- 1. POST: Generate Prorated Razorpay Order ---
// 🔒 Secured: Only Students generate orders to pay for seats
router.post('/create-order', authMiddleware, authorizeRoles('Student'), async (req, res) => {
  try {
...
// --- 2. POST: Verify Signature & Auto-Lock Seat ---
// 🔒 Secured: Only Students can verify payments and lock seats
router.post('/verify-payment', authMiddleware, authorizeRoles('Student'), async (req, res) => {
  try {
    const {
      razorpay_order_id,
      razorpay_payment_id,
    ...
    await newEnrollment.save(); // 👈 Fixed: Removed duplicate save() call here
